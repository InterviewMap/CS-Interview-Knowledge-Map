...
在这之前需要了解一个重要的性能指标 RTT 时间。该指标表示发送端发送数据到接收到对端数据所需的往返时间。
...
1. 连接初始设置 **Congestion Window** 为一
2. 每过一个 RTT 时间就将窗口大小乘二
3. 指数级增长肯定不能没有限制的，所以有一个阈值限制，当发送窗口大小大于阈值时就会启动拥塞避免算法。