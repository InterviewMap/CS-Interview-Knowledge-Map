...
客户端向服务端发送连接请求报文段。该报文段的头部中 `SYN seq=x`。请求发送后，客户端便进入  SYN-SENT 状态，`x` 表示客户端的数据通信初始序号。

**第二次握手**

服务端收到连接请求报文段后，如果同意连接，则会发送一个应答：`SYN seq=y ACK=x+1` ，发送完成后便进入 SYN-RECEIVED 状态。`y` 表示服务端的数据通信初始序号。`ACK=x+1` 表示服务端希望下一个数据报发送序号从 `x+1` 开始。

**第三次握手**

当客户端收到连接同意的应答后，还要向服务端发送一个确认报文段 `ACK=y+1` 。客户端发完这个报文段后便进入ESTABLISHED 状态，服务端收到这个应答后也进入 ESTABLISHED 状态，此时连接建立成功。
